...
struct PianoRollNoteView: View {
    ...
    var onFrameObtained: (CGRect) -> Void

    var noteColor: Color {
        if note.muted {
            return (Color.white).opacity(Double(note.velocity) / 127 / 2)
        } else {
            return (note.color ?? color).opacity(Double(note.velocity) / 127)
        }
    }
    ...
    func snap(note: PianoRollNote, offset: CGSize, lengthOffset: CGFloat = 0.0) -> PianoRollNote {
        ...
        n.start = floor(min(Double(sequenceLength - 1), n.start))
    ...
    var body: some View {
        ...
        ZStack(alignment: .trailing) {
            ZStack(alignment: .leading) {
                GeometryReader { proxy in
                    Rectangle()
                        .foregroundColor(noteColor.opacity((note.selected || hovering || offset != .zero || lengthOffset != 0) ? 1.0 : 0.8))
                        .onAppear {
                            onFrameObtained(proxy.frame(in: .global))
                            print("frame \(proxy.frame(in: .global))")
                        }
                    Text(note.text ?? "")
        ...
        // NOTE: - Explore, why some of the dimension could be negative during selection
        .frame(width: max(0, gridSize.width * CGFloat(note.length)),
               height: max(0, gridSize.height))
