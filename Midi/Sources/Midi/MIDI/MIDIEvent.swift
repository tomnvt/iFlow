// Copyright AudioKit. All Rights Reserved. Revision History at http://github.com/AudioKit/AudioKit/
    ...
    public struct MIDIEvent: MIDIMessage, Equatable {
        ...
        ///
        init(data: [MIDIByte], timeStamp: MIDITimeStamp? = nil) {
            ...
            } else if !data.isEmpty, let command = MIDISystemCommand(rawValue: data[0]) {
                self.data = []
                // is sys command
                if command == .sysEx {
                    for byte in data {
            ...
            } else if !data.isEmpty, let status = MIDIStatusType.from(byte: data[0]) {
                // is regular MIDI status
                let channel = data[0].lowBit
                fillData(status: status, channel: channel, bytes: Array(data.dropFirst()))
            } else if !data.isEmpty, let metaType = MIDICustomMetaEventType(rawValue: data[0]) {
        ...
        ///
        init(noteOn noteNumber: MIDINoteNumber, velocity: MIDIVelocity, channel: MIDIChannel) {
        ...
        ///   - channel:    Channel on which the note appears
        ///
        init(noteOff noteNumber: MIDINoteNumber, velocity: MIDIVelocity, channel: MIDIChannel) {
        ...
        ///
        init(programChange data: MIDIByte, channel: MIDIChannel) {
        ...
        ///
        init(controllerChange controller: MIDIByte, value: MIDIByte, channel: MIDIChannel) {
        ...
        static func midiEventsFrom(packetListPointer: UnsafePointer<MIDIPacketList>) -> [MIDIEvent] {
        ...
        static func generateFrom(bluetoothData: [MIDIByte]) -> [MIDIEvent] {
